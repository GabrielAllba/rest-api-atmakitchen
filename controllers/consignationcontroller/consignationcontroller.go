...
func Search(c *gin.Context) {
	query := c.Query("query")
	var consigns []models.Consignation

	query = strings.ToLower(query)
	result := models.DB.Where("LOWER(name) LIKE ? OR LOWER(address) LIKE ? OR phone_number LIKE ? OR LOWER(bank_account) LIKE ? OR bank_number LIKE ?", "%"+query+"%", "%"+query+"%", "%"+query+"%", "%"+query+"%", "%"+query+"%")

	if err := result.Find(&consigns).Error; err != nil {
		switch err {
	...
	c.JSON(http.StatusOK, gin.H{"consignation": consigns})
}
